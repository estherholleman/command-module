...
describe("plugin-path", () => {
  test("clones a branch to a stable cache path", async () => {
    ...
    const cacheDir = path.join(tempHome, ".cache", "command-module", "branches", "command-module-feat~test-branch")
    const pluginDir = path.join(cacheDir, "plugins", "command-module")
  ...
  test("sanitizes branch names with slashes into stable directory names", async () => {
    ...
    expect(stdout).toContain("command-module-feat~deep~nested~branch")
    expect(stderr).toContain("claude --plugin-dir")
  })
  ...
  test("updates existing checkout on re-run", async () => {
    ...
    const markerPath = path.join(repoRoot, "plugins", "command-module", "MARKER.txt")
    ...
    const cacheDir = path.join(tempHome, ".cache", "command-module", "branches", "command-module-feat~update-test")
    ...
    const cachedMarker = path.join(cacheDir, "plugins", "command-module", "MARKER.txt")
