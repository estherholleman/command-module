...
16. **Willingness to pay** — them talking about *money*: what they spend, frugal vs. splurge, reactions to prices, budget tiers. **Stated WTP is weak evidence** — for the WTP conclusion, lead with **behavioral/market data**: marketplace sales counts + reviews (e.g. Etsy/Amazon listings of comparable items), live competitor/product prices, and paid alternatives they already hire. Collect WTP *quotes* as color, but anchor the verdict on money actually spent. (One comparable listing with hundreds of sales beats a pile of "I'd buy that!" comments.)

For each bucket return a **quote bank**: every entry = verbatim quote · handle · platform · **direct URL** · date · one-line context. The quality bar is absolute — **traceability beats volume**; ten clickable quotes beat thirty unverifiable ones, and a quote that won't byte-verify never enters the bank.

**Volume target & saturation.** Aim for roughly **30–50 byte-verified quotes** per segment (skew higher for large, accessible communities; expect fewer for niche or heavily-walled ones). But **saturation governs the count, not the target**: keep mining while fresh sources still surface new pains/wants/workarounds, and stop once ~two further rounds add nothing new — even if you're under target. If you land well under ~30 because a major source was blocked or the niche is small, say so explicitly as a **coverage note** (it caps confidence; it is not a pass). Never pad with weak or unverifiable quotes to hit a number.
...
- **Sensemaking synthesis** — cluster the quotes, count frequency (soft quant), surface tensions, and for each significant pain give a **problem-worth-solving verdict** (real / nice-to-have / unclear) grounded in time-and-money evidence. For any **WTP / pricing** claim, ground the verdict in **behavioral/market evidence** (sales counts, live prices, paid alternatives), not the volume of stated-WTP quotes. State the verified-quote count and flag if a thin sample or a blocked source caps confidence.
