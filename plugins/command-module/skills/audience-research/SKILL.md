...
4. **Desire is not a problem worth solving.** Trust pains people already spend time or money on, not ones they merely endorse. For willingness-to-pay specifically, **behavioral evidence (sales counts, live prices, paid alternatives) outranks any volume of stated-WTP quotes.**
...
- **Neutral discovery** (Sections A-D). Mine the real communities. For each buyer quote, fetch the exact page and **byte-verify** the quote against the raw page before it's trusted. Target ~30-50 verified quotes per segment, but let **saturation** govern (stop when ~two rounds add nothing new; keep going if themes still appear; flag a thin count as a coverage cap, never pad). For **WTP**, anchor on behavioral/market data (sales counts, prices, paid alternatives) -- quotes are color, not proof.
...
- **Quote volume:** aim for **~30-50 byte-verified quotes** when major sources are accessible; saturation governs the exact count. The first run landed at ~13 only because its richest vein (Reddit) was fully blocked -- treat that as a coverage cap, not the norm. Quality bar stays absolute (no padding); a thin count is a confidence cap to state, not hide. The `quotes.jsonl` store accumulates across passes, so a segment can be thickened by a later top-up run.
