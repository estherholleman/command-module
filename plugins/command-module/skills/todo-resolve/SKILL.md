...
Scan `.context/command-module/todos/*.md` and legacy `todos/*.md`. Partition by status:
...
Residual actionable work from `code-review mode:autofix` after its `safe_auto` pass will already be `ready`.
...
Spawn a `command-module:workflow:pr-comment-resolver` agent per item. Prefer parallel; fall back to sequential respecting dependency order.
...
For large sets, use a scratch directory at `.context/command-module/todo-resolve/<run-id>/` for per-resolver artifacts. Return only completion summaries to parent.
...
Delete completed/resolved todo files from both paths. If a scratch directory was created at `.context/command-module/todo-resolve/<run-id>/`, delete it (unless user asked to inspect).
