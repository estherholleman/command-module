...
```bash
# Count components
echo "Agents: $(ls plugins/command-module/agents/*.md | wc -l)"
echo "Skills: $(ls -d plugins/command-module/skills/*/ 2>/dev/null | wc -l)"

# Validate JSON
cat .claude-plugin/marketplace.json | jq . > /dev/null && echo "✓ marketplace.json valid"
cat plugins/command-module/.claude-plugin/plugin.json | jq . > /dev/null && echo "✓ plugin.json valid"

# Check all HTML files exist
for page in index agents commands skills mcp-servers changelog getting-started; do
  if [ -f "plugins/command-module/docs/pages/${page}.html" ] || [ -f "plugins/command-module/docs/${page}.html" ]; then
...
```bash
git status --porcelain plugins/command-module/docs/
```
  ...
  deploy:
    ...
    steps:
      ...
      - uses: actions/upload-pages-artifact@v3
        with:
          path: 'plugins/command-module/docs'
...
- [ ] Check deployment at https://everyinc.github.io/command-module-plugin/
```
