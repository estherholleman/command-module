...
- `implementation-plan` answers: "How should it be built?"
...
- a path such as `plugins/command-module/skills/`
...
2. **Learnings search** — dispatch `command-module:research:learnings-researcher` with a brief summary of the ideation focus.

3. **Issue intelligence** (conditional) — if issue-tracker intent was detected in Phase 0.2, dispatch `command-module:research:issue-intelligence-analyst` with the focus hint. If a focus hint is present, pass it so the agent can weight its clustering toward that area. Run this in parallel with agents 1 and 2.
