...
6. **Extract the stakeholder's vocabulary lists from the context file**, if present:
   - **Known vocabulary** — terms the stakeholder already uses or understands. These pass through the report unchanged; do not translate or unpack them.
   - **Avoid vocabulary** — internal shorthand, testing/QA vocabulary, or architectural names the stakeholder does not know. These must be rewritten in plain language; the context file should give a suggested replacement for each.

   These lists drive both the drafting rules in Phase 2 and the cold-read persona pass in Phase 3. If the context file does not yet have them, proceed without — but flag it to the user at the end so they can be added when feedback comes in.
...
- **Use the stakeholder's vocabulary lists from the context file as the allowlist and blocklist for terminology.** Terms on the **known vocabulary** list pass through unchanged — do not translate or unpack them, and do not flag them in the cold-read pass. Domain terms the stakeholder uses themselves (for Wolter: "model free", "model based", "growth rate", "acc", "prd") are signal, not jargon, and over-translating them is condescending. Terms on the **avoid vocabulary** list must be rewritten using the suggested replacement (or, if no replacement is suggested, by describing what the thing does in plain language). Anything domain-specific that is not on either list and would not pass for plain everyday language is a candidate to either unpack on first use or rewrite — when in doubt, ask the user during the cold-read pass and offer to add it to the appropriate list for next time.
- **The report is invisible to itself — never report on communication produced for this same stakeholder.** Do not include bullets about producing or delivering communication artifacts whose audience is the reader of this report. That includes this report itself, earlier or later drafts of it, and any other explainer document, briefing, summary, slide deck, video walkthrough, or update produced *for this stakeholder*. The reader is currently reading the artifact being described, or is about to receive it through a separate channel — telling them about it inside the report is recursive and adds no information. Examples to omit: "Wolter-versie van het uitleg-document. Wordt nog herschreven voordat het naar je toekomt." (the reader is reading the report; mentioning the report's own draft state inside it is meta-noise). The test: "Is the audience of this artifact the stakeholder reading the report?" If yes, omit the bullet entirely. Communication artifacts produced for *other* stakeholders may still be relevant and should be left for human judgement — do not auto-omit those.
...
3. **Process and tooling jargon from how we work** — equally invisible to the stakeholder, equally exclusionary. Examples to translate or rewrite, not just delete:
   ...
   - Method-internal shorthand: "growth_blend", "fallback pool", "donor-route", "warping", "auto-end" — these *can* appear if explained the first time they show up in the report, but prefer dropping the name entirely and describing what the thing does. Only introduce the name if the stakeholder will see it across multiple reports and needs a handle for it.
   - Testing and QA vocabulary: "regressietest", "regressie", "unit test", "integratietest", "smoke test", "test coverage" — these describe how *we* validate work, which the stakeholder does not need. Rewrite as what was actually checked, e.g. "we hebben gecontroleerd dat de bestaande voorspellingen niet zijn veranderd", "geverifieerd dat er niets stuk is gegaan elders".
   - Internal architectural shorthand: "cross-route logica", "blend logica", "fallback cascade", "fallback laag", "pipeline" (in the architecture sense), "module X", "component Y" — these are nicknames for parts of the system that only make sense from the inside. Describe what the part *does* for the user / forecast, not the architectural name. "Pipeline" in the deployment sense (CI/CD) belongs in the bullet above; "pipeline" in the architecture sense (a chain of forecasting steps) belongs here.
...
2. **Cold-read persona pass.** Re-read the draft from the seat of the actual stakeholder — someone who has *not* been in the build, who scans rather than reads, and who only knows their own domain. Use the stakeholder's profile and context file to ground the persona (their role, their domain vocabulary, what they have and have not been told before). Apply the stakeholder's **known vocabulary** list as an allowlist (these terms are signal, not jargon, and must not be flagged) and the **avoid vocabulary** list as an auto-flag list (every occurrence is a finding, with the suggested replacement attached). Walk through the draft bullet by bullet and flag every instance of the failure modes below; do not silently rewrite — surface the findings to the user with the offending phrasing and a proposed replacement, then apply the user's choices.
   ...
   - **References to internal events / artifacts / conversations the reader was never part of.** Phrases like "naar aanleiding van de X-bevinding van vandaag", "uit de meeting van maandag", "tijdens de discussie van gisteren", "zoals besproken met Ferry", "n.a.v. de audit", "uit de packaging-bevinding", "vanuit het overleg" assume the reader was present for an internal moment they were not. This is distinct from cross-section anaphora — there is no earlier bullet to anchor back to; the reference simply appears as if shared knowledge. Propose either dropping the reference entirely (motivation often does not need to be in the report) or restating *what* the finding / discussion / decision was, in one short clause, in plain language.
   - **Self-referential communication bullets.** Any bullet about producing or delivering communication aimed at *this* stakeholder — this report itself, an earlier/later draft of it, an explainer document or briefing intended for the reader, a presentation prepared *for* the reader. These are recursive (the reader is reading the artifact being described) and should be flagged for deletion. Examples: "Wolter-versie van het uitleg-document. Wordt nog herschreven voordat het naar je toekomt." Do *not* flag bullets about communication produced for *other* stakeholders — those may still be relevant and should be left to user judgement.
   ...
   - **Avoid-vocabulary terms.** Every term on the stakeholder's `avoid_vocabulary` list (e.g. for Wolter: "regressietest", "cross-route logica", "blend logica", "fallback cascade", "CI/CD", "deploy", "unit test", "PR/merge/branch", "growth_blend"/"growth_bayesian"/"auto-end", "oversprongen"). Attach the suggested replacement from the context file.
   - **Unfamiliar domain terms not on either list.** Anything domain-specific or internal-sounding that is not on the known list and not on the avoid list. Flag with a question to the user: "Does Wolter know this term? If yes, add to known; if no, propose a replacement and add to avoid." This is how the lists grow.

   After the user resolves these findings, apply them in the markdown before moving on to proof.

3. **Update the stakeholder context file with new vocabulary decisions.** During the cold-read pass, the user will tell you which unfamiliar terms the stakeholder does or does not know, and may give corrections like "Wolter does know X" or "Wolter does not know Y". Append these to the appropriate list in `missioncontrol/reports/stakeholders/{stakeholder}-context.md` so the next report inherits the decision. Do this *before* moving on to proof so the file reflects the most recent state. If the context file does not yet have the two lists, create them using the format documented in that file.

4. **Invoke the `proof` skill on the draft.** Pass the file path as the argument. Proof will scan for AI-typical patterns, auto-fix tier 1 items (banned words, filler phrases), and generate a tier 2 findings document for items that need judgment (em dashes, structural rewrites). For Dutch reports, proof loads `references/writing-style-nl.md` automatically.

5. **Wait for the user's tier 2 choices**, then apply them. The skill should not proceed to format generation until the user has resolved the findings (or explicitly skipped them).

6. **Em dash spot-check.** After proof applies fixes, do a final grep for `—` in the markdown. If any remain, replace them with commas, periods, colons, or parentheses based on the surrounding clause — em dashes should never appear in the final output for stakeholders who have flagged them.
...
- **Never reference an internal event the reader was not part of.** "Naar aanleiding van de X-bevinding van vandaag", "uit de meeting van maandag", "tijdens de discussie van gisteren", "zoals besproken", "n.a.v. de audit" assume shared presence the reader does not have. Either drop the reference or restate *what was found / decided* in plain language. Internal motivation is often not needed in the report at all.
- **The report is invisible to itself.** Never include bullets about producing or delivering communication aimed at *this* stakeholder — this report or its drafts, explainer docs / briefings / slides / videos prepared for the reader. The reader is reading the artifact; describing it inside itself is recursive. Communication for *other* stakeholders is not auto-omitted — leave those to user judgement.
...
- **Honour the stakeholder's known and avoid vocabulary lists.** Terms on the known list (for Wolter: "model free", "model based", "growth rate", "acc", "prd", etc.) pass through unchanged — do not translate them. Terms on the avoid list (for Wolter: "regressietest", "cross-route logica", "blend logica", "fallback cascade", "CI/CD", "deploy", "unit test", "growth_blend", "auto-end", "oversprongen", etc.) must be rewritten using the suggested replacement. Update the lists at the end of each report based on user feedback so the next one inherits it.
