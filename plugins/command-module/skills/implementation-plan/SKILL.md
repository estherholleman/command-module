...
- Task command-module:research:repo-research-analyst(Scope: technology, architecture, patterns. {planning context summary})
- Task command-module:research:learnings-researcher(planning context summary)
...
- Task command-module:research:best-practices-researcher(planning context summary)
- Task command-module:research:framework-docs-researcher(planning context summary)
...
- Task command-module:workflow:spec-flow-analyzer(planning context summary, research findings)
...
- `command-module:workflow:spec-flow-analyzer` for missing user flows, edge cases, and handoff gaps
- `command-module:research:repo-research-analyst` (Scope: `architecture, patterns`) for repo-grounded patterns, conventions, and implementation reality checks

**Context & Research / Sources & References gaps**
- `command-module:research:learnings-researcher` for institutional knowledge and past solved problems
- `command-module:research:framework-docs-researcher` for official framework or library behavior
- `command-module:research:best-practices-researcher` for current external patterns and industry guidance
- Add `command-module:research:git-history-analyzer` only when historical rationale or prior art is materially missing

**Key Technical Decisions**
- `command-module:review:architecture-strategist` for design integrity, boundaries, and architectural tradeoffs
- Add `command-module:research:framework-docs-researcher` or `command-module:research:best-practices-researcher` when the decision needs external grounding beyond repo evidence

**High-Level Technical Design**
- `command-module:review:architecture-strategist` for validating that the technical design accurately represents the intended approach and identifying gaps
- `command-module:research:repo-research-analyst` (Scope: `architecture, patterns`) for grounding the technical design in existing repo patterns and conventions
- Add `command-module:research:best-practices-researcher` when the technical design involves a DSL, API surface, or pattern that benefits from external validation

**Implementation Units / Verification**
- `command-module:research:repo-research-analyst` (Scope: `patterns`) for concrete file targets, patterns to follow, and repo-specific sequencing clues
- `command-module:review:pattern-recognition-specialist` for consistency, duplication risks, and alignment with existing patterns
- Add `command-module:workflow:spec-flow-analyzer` when sequencing depends on user flow or handoff completeness

**System-Wide Impact**
- `command-module:review:architecture-strategist` for cross-boundary effects, interface surfaces, and architectural knock-on impact
- Add the specific specialist that matches the risk:
  - `command-module:review:performance-oracle` for scalability, latency, throughput, and resource-risk analysis
  - `command-module:review:security-sentinel` for auth, validation, exploit surfaces, and security boundary review
  - `command-module:review:data-integrity-guardian` for migrations, persistent state safety, consistency, and data lifecycle risks

**Risks & Dependencies / Operational Notes**
- Use the specialist that matches the actual risk:
  - `command-module:review:security-sentinel` for security, auth, privacy, and exploit risk
  - `command-module:review:data-integrity-guardian` for persistent data safety, constraints, and transaction boundaries
  - `command-module:review:data-migration-expert` for migration realism, backfills, and production data transformation risk
  - `command-module:review:deployment-verification-agent` for rollout checklists, rollback planning, and launch verification
  - `command-module:review:performance-oracle` for capacity, latency, and scaling concerns
...
Artifact-backed mode uses a per-run scratch directory under `.context/command-module/ce-plan/deepen/`.
