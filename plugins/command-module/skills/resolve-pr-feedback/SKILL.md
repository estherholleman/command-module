...
**For review threads** (`review_threads`): Spawn a `command-module:workflow:pr-comment-resolver` agent for each.
...
Spawn a single `command-module:workflow:pr-comment-resolver` agent for the thread. Then follow the same commit -> push -> reply -> resolve flow as Full Mode steps 5-6.
