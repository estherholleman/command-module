...
- **Write a run artifact** under `.context/command-module/ce-review/<run-id>/` summarizing findings, applied fixes, residual actionable work, and advisory outputs.
...
- **Never edit files or externalize work.** Do not write `.context/command-module/ce-review/<run-id>/`, do not create todo files, and do not commit, push, or create a PR.
...
| Agent | Focus |
|-------|-------|
| `command-module:review:correctness-reviewer` | Logic errors, edge cases, state bugs, error propagation |
| `command-module:review:testing-reviewer` | Coverage gaps, weak assertions, brittle tests |
| `command-module:review:maintainability-reviewer` | Coupling, complexity, naming, dead code, abstraction debt |
| `command-module:review:project-standards-reviewer` | CLAUDE.md and AGENTS.md compliance -- frontmatter, references, naming, portability |
| `command-module:review:agent-native-reviewer` | Verify new features are agent-accessible |
| `command-module:research:learnings-researcher` | Search docs/solutions/ for past issues related to this PR |

**Cross-cutting conditional (selected per diff):**

| Agent | Select when diff touches... |
|-------|---------------------------|
| `command-module:review:security-reviewer` | Auth, public endpoints, user input, permissions |
| `command-module:review:performance-reviewer` | DB queries, data transforms, caching, async |
| `command-module:review:api-contract-reviewer` | Routes, serializers, type signatures, versioning |
| `command-module:review:data-migrations-reviewer` | Migrations, schema changes, backfills |
| `command-module:review:reliability-reviewer` | Error handling, retries, timeouts, background jobs |
| `command-module:review:adversarial-reviewer` | Diff >=50 changed non-test/non-generated/non-lockfile lines, or auth, payments, data mutations, external APIs |

**Stack-specific conditional (selected per diff):**

| Agent | Select when diff touches... |
|-------|---------------------------|
| `command-module:review:dhh-rails-reviewer` | Rails architecture, service objects, session/auth choices, or Hotwire-vs-SPA boundaries |
| `command-module:review:kieran-rails-reviewer` | Rails application code where conventions, naming, and maintainability are in play |
| `command-module:review:kieran-python-reviewer` | Python modules, endpoints, scripts, or services |
| `command-module:review:kieran-typescript-reviewer` | TypeScript components, services, hooks, utilities, or shared types |
| `command-module:review:julik-frontend-races-reviewer` | Stimulus/Turbo controllers, DOM events, timers, animations, or async UI flows |

**CE conditional (migration-specific):**

| Agent | Select when diff includes migration files |
|-------|------------------------------------------|
| `command-module:review:schema-drift-detector` | Cross-references schema.rb against included migrations |
| `command-module:review:deployment-verification-agent` | Produces deployment checklist with SQL verification queries |
...
Use the repository portion of the returned PR URL as `<base-repo>` (for example, `EveryInc/command-module-plugin` from `https://github.com/EveryInc/command-module-plugin/pull/348`).
...
2. Filter to those whose directory is an ancestor of at least one changed file. A standards file governs all files below it (e.g., `plugins/command-module/AGENTS.md` applies to everything under `plugins/command-module/`).
...
- In interactive and autofix modes, write a per-run artifact under `.context/command-module/ce-review/<run-id>/` containing:
