...
function resolveBundledMcporterConfigPath(): string | undefined {
  try {
    ...
    const candidates = [
      path.join(extensionDir, "..", "pi-resources", "command-module", "mcporter.json"),
      path.join(extensionDir, "..", "command-module", "mcporter.json"),
    ]
...
function resolveMcporterConfigPath(cwd: string, explicit?: string): string | undefined {
  ...
  const projectPath = path.join(cwd, ".pi", "command-module", "mcporter.json")
  if (fs.existsSync(projectPath)) return projectPath

  const globalPath = path.join(os.homedir(), ".pi", "agent", "command-module", "mcporter.json")
