...
That doc covers the standing release PR, component ownership across `cli`, `compound-engineering`, `coding-tutor`, and `marketplace`, and the GitHub Releases model for published release notes. This document stays narrower: it is the plugin-scoped reminder for contributors changing `plugins/command-module/**`.
...
1. **Avoid release bookkeeping in normal PRs**
   - Do not manually bump `plugins/command-module/.claude-plugin/plugin.json`
...
- [ ] No manual version bump in `plugins/command-module/.claude-plugin/plugin.json`
...
- Plugin version is release-owned: `plugins/command-module/.claude-plugin/plugin.json`
...
1. Create the agent file in `plugins/command-module/agents/[category]/`
2. Update `plugins/command-module/README.md`
...
- `plugins/command-module/README.md`
- `package.json`
- `CHANGELOG.md`
- `docs/solutions/workflow/manual-release-please-github-releases.md`
